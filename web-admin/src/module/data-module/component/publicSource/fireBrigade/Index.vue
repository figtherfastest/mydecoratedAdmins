  ...
  <el-container>
    <el-main>
      ...
      <ele-data-tables :server-params="searchObj"
        ...
        <el-table-column label="中文全称" prop="fullname" sortable="custom"/>
        <el-table-column label="所在区域" prop="areaName" sortable="custom"/>
        <el-table-column label="地址" prop="address" sortable="custom"/>
        <el-table-column label="消防队级别" prop="fireBrigadeGrade" sortable="custom"/>
        <el-table-column label="消防队电话" prop="fireBrigadeType" sortable="custom"/>
...
<script>
  ...
  export default class Index extends Vue {
    ...
    regions
    @fireBrigadeModule.State('url')
    ajax
    edit (params) {
      this.$router.push({ name: 'fireBrigade', params })
    }

    add () {
      this.$router.push({ name: 'fireBrigade', params: { id: 'new' } })
    }
  }
</script>
<style lang="less">
