  ...
  <el-container>
    <el-main>
      ...
      <ele-data-tables :server-params="searchObj"
        ...
        <el-table-column label="医院全称" prop="fullname" sortable="custom"/>
        <el-table-column label="所在区域" prop="areaName" sortable="custom"/>
        <el-table-column label="地址" prop="address" sortable="custom"/>
        <el-table-column label="医院等级" prop="hospitalGrade" sortable="custom"/>
        <el-table-column label="医院类别" prop="hospitalType" sortable="custom"/>
...
<script>
  ...
  export default class Index extends Vue {
    ...
    @hospitalModule.State('url')
    ajax
    @regionModule.Getter('regions')
    regions
    edit (params) {
      this.$router.push({ name: 'hospital', params })
    }

    add () {
      this.$router.push({ name: 'hospital', params: { id: 'new' } })
    }
  }
</script>
<style lang="less">
