...
<script>
  ...
  const carMessageModule = namespace('data/car')
  @Component({
    components: {}
  })
  export default class CarMessage extends Vue {
    @Prop({default: () => 'new'})
    id
    @Prop({default: () => 'new'})
    companyId
    ...
    @carMessageModule.Action('save')
    saveData

    @carMessageModule.Action('get')
    getData

    @carMessageModule.Action('update')
    updateData

    save () {
      this.data.company = {id: this.companyId}
