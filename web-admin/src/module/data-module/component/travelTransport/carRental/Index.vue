    ...
    <el-main>
      ...
      <ele-data-tables :server-params="searchObj"
        ...
        <el-table-column label="是否认证">
          <template slot-scope="scope">{{ scope.row.authenticationed?'是':'否' }}</template>
        </el-table-column>
...
<script>
  ...
  const carRentalModule = namespace('data/carRental')
  ...
  export default class Index extends Vue {
    ...
    regions
    @carRentalModule.State('url')
    ajax

    edit (params) {
      this.$router.push({name: 'carRental', params})
    }

    add () {
      this.$router.push({name: 'carRental', params: {id: 'new'}})
    }
  }
</script>
<style lang="less">
