    ...
    <el-main>
      <el-form :inline="true" :model="searchObj" class="clear-float">
        <el-row
          ...
          <el-col :span="4">
            <el-input v-model="searchObj .chineseName" placeholder="请输入中文名称"/>
          </el-col>
          <el-col :span="4">
      ...
      <ele-data-tables :server-params="searchObj"
        ...
        <el-table-column label="中文名称" prop="chineseName" sortable="custom"/>
        <el-table-column label="所在区域" prop="areaName" sortable="custom"/>
        <el-table-column label="地址" prop="address" sortable="custom"/>
        <el-table-column label="等级" prop="level" sortable="custom"/>
...
<script>
  ...
  const passengerStationModule = namespace('data/passengerStation')
  ...
  export default class Index extends Vue {
    ...
    regions
    @passengerStationModule.State('url')
    ajax

    edit (params) {
      this.$router.push({ name: 'passengerStation', params })
    }

    add () {
      this.$router.push({ name: 'passengerStation', params: { id: 'new' } })
    }
  }
</script>
<style lang="less">
