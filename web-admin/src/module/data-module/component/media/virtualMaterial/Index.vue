  ...
  <el-container>
    <el-main>
      ...
      <ele-data-tables :server-params="searchObj"
                       :ajax="ajax">
        <el-table-column label="素材标题" prop="materialTitle" sortable="custom"/>
        <el-table-column label="类型" prop="type" sortable="custom"/>
        <el-table-column label="素材分类" prop="theMaterialClassification" sortable="custom"/>
...
<script>
  ...
  export default class Index extends Vue {
    ...
    @virtualMaterialModule.State('url')
    ajax

    edit (params) {
      this.$router.push({ name: 'virtualMaterial', params })
    }

    add () {
      this.$router.push({ name: 'virtualMaterial', params: { id: 'new' } })
    }
  }
</script>
<style lang="less">
