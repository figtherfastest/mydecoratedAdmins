  ...
  <el-container>
    <el-main>
      ...
      <ele-data-tables :server-params="searchObj"
                       :ajax="ajax">
        <el-table-column label="图片标题" prop="pictureCaptions" sortable="custom"/>
        <el-table-column label="类型" prop="type" sortable="custom"/>
        <el-table-column label="图片格式" prop="imageFormat" sortable="custom"/>
...
<script>
  ...
  export default class Index extends Vue {
    ...
    @imageDatabaseModule.State('url')
    ajax

    edit (params) {
      this.$router.push({ name: 'imageDatabase', params })
    }

    add () {
      this.$router.push({ name: 'imageDatabase', params: { id: 'new' } })
    }
  }
</script>
<style lang="less">
